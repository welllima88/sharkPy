...
class wire_writer(object):
    ...
    def start(self, write_interface_list):
        ...
        try:
            t=Thread(target=self.run, args=(write_interface_list, self.command_queue, self.status_queue, self.command_failure))
            t.daemon=True
            t.start()
    ...
    def run(self, write_interface_list, command_queue, status_queue, fail_event):
        lib_directories=[site.getsitepackages()[0],site.getsitepackages()[0]]
