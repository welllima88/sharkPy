...
/*
 * sharkPy.c is a modified version of tshark.c compiled as a shared lib
 * that exposes dissection and object export interfaces to sharkPy
...
/*
 * Must be set to directory containing dumpcap for
 * wire_dissect module to work.
 */
char *sharkPy_program_dir;
...
static gboolean really_quiet = TRUE; /* Unlike tshark, sharkPy sets this to true by default. */
...
static gboolean sharkPy_print_packet(capture_file *cf, epan_dissect_t *edt);
...
{
	copy_out_attribute=func;
}

void
sharkPy_set_prog_dir(const char *name){
	sharkPy_program_dir=(char *)name;
}
...
#define OPTSTRING "+2" OPTSTRING_CAPTURE_COMMON "C:d:e:E:F:gG:hH:" "K:lnN:o:O:PqQr:R:S:t:T:u:vVw:W:xX:Y:z:"
  ...
  /*
   * Set dumpcap program directory
   */
  sharkPy_set_prog_dir(argv[0]);
  ...
  /*
   * Always exporting packet dissection details.
   */
  print_summary = FALSE;
  print_details = TRUE;
  print_packet_info = TRUE;
  do_dissection = TRUE;
  tshark_debug("sharkPy: do_dissection = %s", do_dissection ? "TRUE" : "FALSE");
...
/** Send All g_log messages to our own handler **/
  ...
  */
  ...
  tshark_debug("sharkPy reading preferences");
  ...
  if (capture_option_specified)
    cmdarg_err("This version of sharkPy was not built with support for capturing packets.");
#endif
  if (arg_error) {
    print_usage(stderr);
    return 1;
  ...
  if (cf_name) {
    tshark_debug("sharkPy: Opening capture file: %s", cf_name);
    /*
    ...
    tshark_debug("sharkPy: invoking load_cap_file() to process the packets");
    TRY {
#ifdef HAVE_LIBPCAP
    ...
    tshark_debug("sharkPy: no capture file specified");
    ...
    tshark_debug("sharkPy: performing live capture");
    /*
    ...
    gboolean out_file_name_res, int max_packet_count, gint64 max_byte_count)
{
  ...
  if (save_file != NULL) {
    ...
    tshark_debug("sharkPy: snapshot_length = %d", snapshot_length);
    ...
    if (shb_hdr->shb_user_appl == NULL) {
        ...
        shb_hdr->shb_user_appl = g_strdup_printf("sharkPy (Wireshark) %s", "blah");
    }

    if (linktype != WTAP_ENCAP_PER_PACKET &&
        out_file_type == WTAP_FILE_TYPE_SUBTYPE_PCAP) {
        tshark_debug("sharkPy: writing PCAP format to %s", save_file);
        ...
        tshark_debug("sharkPy: writing format type %d, to %s", out_file_type, save_file);
        if (strcmp(save_file, "-") == 0) {
          /* Write to the standard output. */
  ...
  if (perform_two_pass_analysis) {
    ...
    tshark_debug("sharkPy: perform_two_pass_analysis, do_dissection=%s", do_dissection ? "TRUE" : "FALSE");
    ...
    if (do_dissection) {
      ...
      tshark_debug("sharkPy: create_proto_tree = %s", create_proto_tree ? "TRUE" : "FALSE");
    ...
    tshark_debug("sharkPy: reading records for first pass");
    while (wtap_read(cf->wth, &err, &err_info, &data_offset)) {
      if (process_packet_first_pass(cf, edt, data_offset, wtap_phdr(cf->wth),
        ...
        if ( (--max_packet_count == 0) || (max_byte_count != 0 && data_offset >= max_byte_count)) {
          tshark_debug("sharkPy: max_packet_count (%d) or max_byte_count (%" G_GINT64_MODIFIER "d/%" G_GINT64_MODIFIER "d) reached",
    ...
    tshark_debug("sharkPy: done with first pass");
    ...
    for (framenum = 1; err == 0 && framenum <= cf->count; framenum++) {
      ...
      if (wtap_seek_read(cf->wth, fdata->file_off, &phdr, &buf, &err,
                         &err_info)) {
        tshark_debug("sharkPy: invoking process_packet_second_pass() for frame #%d", framenum);
        if (process_packet_second_pass(cf, edt, fdata, &phdr, &buf,
          ...
          /* Either there's no read filtering or this packet passed the
             ...
             this packet out. */
          if (pdh != NULL) {
            tshark_debug("sharkPy: writing packet #%d to outfile", framenum);
            if (!wtap_dump(pdh, &phdr, ws_buffer_start_ptr(&buf), &err, &err_info)) {
              /* Error writing to a capture file */
              tshark_debug("sharkPy: error writing to a capture file (%d)", err);
              switch (err) {
    ...
    tshark_debug("sharkPy: done with second pass");
  }
  else {
    /* !perform_two_pass_analysis */
    framenum = 0;

    tshark_debug("sharkPy: perform one pass analysis, do_dissection=%s", do_dissection ? "TRUE" : "FALSE");
    ...
    while (wtap_read(cf->wth, &err, &err_info, &data_offset)) {
      ...
      tshark_debug("sharkPy: processing packet #%d", framenum);
  ...
  if (err != 0) {
    tshark_debug("sharkPy: something failed along the line (%d)", err);
    /*
               ...
               const guchar *pd, guint tap_flags)
{
  ...
  if (passed) {
    ...
    /* this must be set after sharkPy_print_packet() [bug #8160] */
...
sharkPy_print_packet(capture_file *cf, epan_dissect_t *edt)
{
  (void) cf;
  attribute end = {0};
  end.type = EXPORTED_FINI;
  proto_tree_write_protobuf(edt, stdout);

  /*
   * Send EXPORT_FINI node to denote end of frame.
   */
  copy_out_attribute(&end);
