...
static void proto_tree_protobuf( proto_node *node, gpointer data )
{
    ...
    *pdata->id += 1;
	...
	/* [3] Get labels/test descriptions/text representations of data */
   ...
   /*
    * [7] Iterate over all child-nodes, rinse and repeat. Skipped if error
    ...
    */
    if ( node->first_child != NULL ) {
        ...
        /* Free export buffer. Data has been written out and buffer is no longer required. */
        if ( NULL != fstring )
        {
                g_free( fstring );
                fstring = NULL;
