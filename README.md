...
This is a alpha release code. Existing code should work. Still need to write wire_reader.py. Still need to write code that will also compile on Windows.

Currently, will run on newish releases of Centos and Ubuntu 64-bit Linux.  

##SharkPy allows users to
...
minimizing unwanted code overhead. You can look at all the underlying code if you want to. However, you only need to understand a couple/few commands and a couple of data structures to use this module.

###Important note 
Using this code to create an IPS (or other network defense programs) is NOT recommended. These modules use Wireshark dissection libraries which have been known to contain memory leaks and other problematic issues. Code is provided "as-is" with NO WARRANTIES expressed or implied. 

###CAVEAT:
A familiarity with using Wireshark is assumed. Best to learn Wireshark basics before using this module.
...
##Packets as Python dictionaries
pktDict={}
...
***************************************************************************************************
For example, We can directly access the fValues of each node as follows.
...
Each node structure has attributes that discribe node details. The attribute names:
...
dictionary object. This allows you to go directly to the data elements that you are interested in using the node 'abbrev' fields as the dictionary keys. In other words, no need to walk a tree.

Since 'abbrevs' are not always unique, the abbrev field indexes a list of nodes. This could be a list
of one element. However, this will not always be the case. See ip.host example.
